...
import os
import pandas as pd
from logic.deckbuilder import build_deck
...
def list_yaml_presets():
    return [
        f for f in os.listdir(CONFIG_PRESETS_DIR)
...
def list_user_inventories():
    ...
    return [f for f in os.listdir(USER_UPLOADS_DIR) if f.endswith(".txt") or f.endswith(".csv")]
...
def deck_output_tab():
    ...
    inventory_upload = gr.File(
        label="Upload Inventory File (.txt or .csv)",
        file_types=[".txt", ".csv"]
    )
    ...
    # Arena export output
    arena_export = gr.Textbox(label="MTG Arena Import String", lines=4, interactive=False)
    ...
    def run_deckbuilder(config_file, inventory_file):
        if not config_file or not inventory_file:
            return "Please select both a config and an inventory.", pd.DataFrame(), ""
        ...
        for card in deck.get("cards", []):
            pt = f"{card.get('power','')}/{card.get('toughness','')}" if card.get("power") or card.get("toughness") else ""
            legalities = ", ".join(f"{fmt}:{stat}" for fmt, stat in card.get("legalities", {}).items())
            card_rows.append([
                card.get("name", ""),
                card.get("type", ""),
                card.get("rarity", ""),
        ...
        df = pd.DataFrame(card_rows, columns=[
            "Name", "Type", "Rarity", "Legalities", "CMC", "Cost",
            "Colors", "Power/Toughness", "Qty", "Card Text"
        ])
