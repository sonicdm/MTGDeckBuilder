...
def _distribute_basic_lands(
    ...
    logger.debug(f"[DEBUG] selected_cards before distributing basics: { {k: v.owned_qty for k,v in selected_cards.items()} }")
    ...
    for color, count in basics_distribution.items():
        ...
        if card.name in selected_cards:
            ...
            selected_cards[card.name] = card
    logger.debug(f"[DEBUG] selected_cards after distributing basics: { {k: v.owned_qty for k,v in selected_cards.items()} }")
...
def _fill_categories(categories, repo, selected_cards, mana_min, mana_max, max_copies, deck_size, weighting_rules=None, inventory_items=None):
...
def _fill_with_any(repo, selected_cards, deck_size, mana_min, mana_max, max_copies, weighting_rules=None, inventory_items=None):
    ...
    logger.debug(f"Filling up to deck size with any cards, starting with {sum(card.owned_qty for card in selected_cards.values())} cards")
    last_count = -1
    fill_pass = 0
    while sum(card.owned_qty for card in selected_cards.values()) < deck_size and all_cards:
        ...
        for idx, card in enumerate(all_cards):
            ...
            if idx % 100 == 0 and idx > 0:
                logger.debug(f"Fill loop pass {fill_pass}, idx={idx}, selected_cards={sum(card.owned_qty for card in selected_cards.values())}")
        fill_pass += 1
        logger.debug(f"Fill pass {fill_pass} complete, added_this_pass={added_this_pass}, selected_cards={sum(card.owned_qty for card in selected_cards.values())} cards")
        if added_this_pass == 0:
            logger.debug(f"No more unique cards to add, stopping fill at {sum(card.owned_qty for card in selected_cards.values())} cards after {fill_pass} passes.")
